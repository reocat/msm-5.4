...
static int ipcam_soc_snd_probe(struct platform_device *pdev)
{
	int errorCode = 0;
	...
	errorCode = snd_soc_register_card(&snd_soc_card_ipcam);
	if (errorCode) {
		dev_err(&pdev->dev, "snd_soc_register_card failed (%d)\n", errorCode);
		goto ipcam_soc_snd_probe_exit0;
	}

	return 0;

ipcam_soc_snd_probe_exit0:
	gpio_free(mic_gpio);
ipcam_soc_snd_probe_exit1:
	gpio_free(spk_gpio);
ipcam_soc_snd_probe_exit2:
	return errorCode;
}

static int ipcam_soc_snd_remove(struct platform_device *pdev)
{
	snd_soc_unregister_card&snd_soc_card_ipcam);
	gpio_free(spk_gpio);
	gpio_free(mic_gpio);
...
static struct platform_driver ipcam_soc_snd_driver = {
	.driver = {
		.name = "snd_soc_card_ipcam",
		.owner = THIS_MODULE,
	},
	.probe = ipcam_soc_snd_probe,
	.remove = ipcam_soc_snd_remove,
};

module_platform_driver(ipcam_soc_snd_driver);
