 ...
 *  2014/07/03 - [Ken He] modify for s2l RDB board
 *
	...
	struct snd_pcm_hw_params *params)
{
	...
	if (clk_fmt == 0) {
		errorCode = snd_soc_dai_set_sysclk(codec_dai, ES8388_MCLK, clk.mclk, 0);
	} else if (clk_fmt == 1) {
		errorCode = snd_soc_dai_set_sysclk(codec_dai, ES8388_BCLK_IN, clk.bclk, 0);
	} else if (clk_fmt == 2) {
		errorCode = snd_soc_dai_set_sysclk(codec_dai, ES8388_MCLK_IN_BCLK_OUT, clk.mclk, 0);
	} else {
		pr_err("clk_fmt is wrong\n");
		return -1;
	}
...
/* ambevk machine audio map (connections to es8388 pins) */
...
static int ambevk_es8388_init(struct snd_soc_pcm_runtime *rtd)
{
	return 0;
}
...
static struct snd_soc_dai_link ambevk_dai_link = {
	.name = "es8388",
	.stream_name = "es8388 PCM",
	.codec_dai_name = "es8388-hifi",
	.init = ambevk_es8388_init,
...
static const struct of_device_id ambevk_dt_ids[] = {
	{ .compatible = "ambarella,s2lrdb-es8388", },
	{ /* sentinel */ }
};
...
MODULE_DESCRIPTION("Amabrella RDB with ES8388 Codec for ALSA");
MODULE_LICENSE("GPL");
MODULE_ALIAS("snd-soc-s2lrdb");
