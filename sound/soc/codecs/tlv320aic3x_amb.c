			   ...
			   const struct i2c_device_id *id)
{
	...
	if (pdata) {
		...
		aic3x->pwr_reset = of_get_named_gpio_flags(np, "gpio-pwr", 0, &flags);
		aic3x->pwr_active = !!(flags & OF_GPIO_ACTIVE_LOW);

		if(aic3x->pwr_reset < 0 || !gpio_is_valid(aic3x->pwr_reset)) {
			   dev_err(&i2c->dev, "Power pin is invalid,"
							"If no pwr gpio is needed, ignore this\n");
			   aic3x->pwr_reset = -1;
		} else {
			ret = gpio_request(aic3x->pwr_reset, "tlv320aic3x pwr");
			if (ret != 0)
				   goto err;
			gpio_direction_output(aic3x->pwr_reset, aic3x->pwr_active);
		}
