...
static int ak4951_writeMask(struct snd_soc_component *, u16, u16, u16);
...
static int ak4951_set_pll(u32 *pll, int clk_id,int freq)
{
...
struct snd_soc_component_driver soc_component_dev_ak4951 = {
	.probe			= ak4951_probe,
...
EXPORT_SYMBOL_GPL(soc_component_dev_ak4951);
                            ...
                            const struct i2c_device_id *id)
{
	...
	ret = snd_soc_register_component(&i2c->dev,
			&soc_component_dev_ak4951, &ak4951_dai[0], ARRAY_SIZE(ak4951_dai));
	if (ret < 0){
		kfree(ak4951);
