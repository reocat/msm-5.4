	...
	const struct i2c_device_id *id)
{
	...
	ret = of_get_named_gpio(np, "gpio-pwr", 0);
	if ((ret > 0) && gpio_is_valid(ret)) {
		devm_gpio_request_one(&i2c->dev, ret, GPIOF_OUT_INIT_HIGH, "tas2505 pwr");
	}

	ret = of_get_named_gpio(np, "gpio-reset", 0);
	if ((ret > 0) && gpio_is_valid(ret)) {
		devm_gpio_request_one(&i2c->dev, ret, GPIOF_OUT_INIT_HIGH, "tas2505 reset");
	}
