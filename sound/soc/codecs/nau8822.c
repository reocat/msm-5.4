...
//export from Audio Codec GUI 3.0
...
{0x7, 0x0},
{0x8, 0x80},
{0x9, 0x0},  // and 0x0D(JCTRL2) is 0x0021
{0xa, 0xc},
{0xb, 0x1ff},
{0xc, 0x1ff},
{0xd, 0x021}, // Jack Detection 0x09  .2010.7.2 // Headphone and speaker high
{0xe, 0x108}, 
{0xf, 0x1ff},
{0x10, 0x1ff},
{0x11, 0x6},
...
{0x26, 0x93},
{0x27, 0xe9},
//[aux2aux/Mic Bias Mode] 
{0x28, 0x0},
//[aux2aux/3D Control] 
{0x29, 0x0},
//[aux2aux/Input Output Mixer] 
{0x2b, 0x10},
{0x2c, 0x30},
{0x2d, 0x50},
{0x2e, 0x10},
...
static const struct snd_kcontrol_new nau8822_snd_controls[] = {
  ...
  SOC_SINGLE("Jack Detection Enable", NAU8822_JACK_DETECT_CONTROL_2, 6, 1, 0),
 ...
 */
static int nau8822_configure_pll(struct snd_soc_codec *codec, int pll_id, int source,
				 ...
				 int div_id, int div)
{
	...
	case NAU8822_OPCLKRATE:
		...
		if (nau8822->f_mclk)
			 ...
			 */
			ret = nau8822_configure_pll(codec, 0, 0, 0, 0);
		break;
			    ...
			    struct snd_soc_dai *dai)
{
	...
	if (nau8822->sysclk == NAU8822_MCLK) {
		...
		if (!nau8822->f_pllout) {
			...
			int ret = nau8822_configure_pll(codec, 0, 0, 0, 0);
				      ...
				      const struct i2c_device_id *id)
{
	...
	if (nau8822 == NULL)
    {
        printk("nau8822 devm_kzalloc failed\n");
		return -ENOMEM;
    }
