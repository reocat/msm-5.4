...
#define MMC_KEY_SIZE            (256/8)*4 /* bytes */
#define XFER_KEY_SIZE           (256/8)*4 /* bytes */
 ...
 */
unsigned char g_uektk_caam_keymod[] =
{
  0xAE, 0x89, 0x9F, 0x28, 0xB8, 0xEE, 0x32, 0x10,
  0x1E, 0xC6, 0xFC, 0x2D, 0x9B, 0xE9, 0x58, 0x30
};
...
static void clear_info ( struct f1_mmc_key_dev *pdev )
{
  unsigned long flags;
  ...
  pdev->have_raw_key = 0;
  pdev->have_xfer_key = 0;
  pdev->have_xfer_blob = 0;

  CLEAR_ARRAY ( pdev->cpu_blob );
  CLEAR_ARRAY ( pdev->raw_key );
  CLEAR_ARRAY ( pdev->xfer_key );
  CLEAR_ARRAY ( pdev->xfer_blob );
                               ...
                               size_t count )
{
  ...
  unsigned char zero_key[MMC_KEY_SIZE];
  CLEAR_ARRAY ( zero_key );
  if ( !memcmp(zero_key, buf, MMC_KEY_SIZE) )
    {
    clear_info ( pdev );
    return count;
    }
...
static ssize_t show_raw_key ( struct device *linuxdev,
                               struct device_attribute *attr,
                               char *buf )
{
  struct f1_mmc_key_dev *pdev = dev_to_key_dev ( linuxdev );
  ...
  if ( pdev->have_raw_key )
    {
    rval = sizeof(pdev->raw_key);
    memcpy ( buf, pdev->raw_key, sizeof(pdev->raw_key) );
    } 
...
static DEVICE_ATTR ( raw_key, 0644, show_raw_key, store_raw_key );
...
int get_f1_mmc_prod_key ( unsigned keynum,
                     unsigned keylen,
                     unsigned char *key )
{
                      ...
                      unsigned inst )
{
  ...
  sprintf ( tempstr, "f1-mmc-prod-key%u", inst );
