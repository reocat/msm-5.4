...
static struct rpmsg_channel *rpdev_vffs;
...
int vffs_rpmsg_exec(struct vffs_msg *msg, int len)
{
	 ...
	 */
	xfr_get(xfr);
	rpmsg_send(rpdev_vffs, msg, len + sizeof(struct vffs_msg));
...
int vffs_rpmsg_fread(char *buf, unsigned int size, int fp)
{
	...
	arg->buf = (char *)ambalink_virt_to_phys((u32)buf);
	arg->size = size;
	arg->fp = fp;
...
int vffs_rpmsg_fstat(const char *path, struct vffs_stat *ffs_stat)
{
	...
	msg->parameter[0] = ambalink_virt_to_phys((u32)ffs_stat);
	...
	ambcache_inv_range((void *) ffs_stat, sizeof(ffs_stat));
#endif
			...
			struct vffs_fsfind *fsfind)
{
	...
	arg->attr = attr;
	arg->res = (void *)ambalink_virt_to_phys((u32)fsfind);
...
int vffs_rpmsg_fsnext(struct vffs_fsfind *fsfind)
{
	...
	msg->parameter[1] = ambalink_virt_to_phys((u32)fsfind);
...
int vffs_rpmsg_getdev(const char *path, struct vffs_getdev *ffs_getdev)
{
	...
	msg->parameter[0] = ambalink_virt_to_phys((u32)ffs_getdev);
	strcpy((char *)&msg->parameter[1], path);
...
int vffs_proc_fs_actual_read(struct file *filp, char *buf, size_t count, loff_t *offp)
{
	...
	if (rval < 0) {
		...
		goto done;
	        ...
	        /* In SMP BOSS, the cache is synced by SCU. */
	        /* We can't invalidate the cache otherwise the data will be missing. */
		ambcache_inv_range((void *) rbuf, count);
#endif
		len = rval;
...
static int vffs_proc_fs_actual_write(struct file *filp, const char *buf, size_t count,loff_t *offp)
{
	...
	/* In SMP BOSS, the cache is synced by SCU. */
	/* We can't invalidate the cache otherwise the data will be missing. */
	ambcache_clean_range((void *) wbuf, len);
#endif
				   ...
				   size_t count, loff_t *data)
{
	...
	/* In SMP BOSS, the cache is synced by SCU. */
	/* We can't invalidate the cache otherwise the data will be missing. */
	ambcache_clean_range((void *) filename, strlen(filename) + 1);
				     ...
				     size_t count, loff_t *data)
{
	...
	/* In SMP BOSS, the cache is synced by SCU. */
	/* We can't invalidate the cache otherwise the data will be missing. */
	ambcache_clean_range((void *) pathname, strlen(pathname) + 1);
#endif
	/* Try to obtain a file descriptor */
				     ...
				     size_t count, loff_t *data)
{
	...
	/* In SMP BOSS, the cache is synced by SCU. */
	/* We can't invalidate the cache otherwise the data will be missing. */
	ambcache_clean_range((void *) drive, strlen(drive) + 1);
#endif
	...
	/* In SMP BOSS, the cache is synced by SCU. */
	/* We can't invalidate the cache otherwise the data will be missing. */
	ambcache_inv_range((void *) &last_ffs_getdev, sizeof(last_ffs_getdev));
#endif
	return count;
}
				     ...
				     size_t count, loff_t *data)
{
	...
	/* In SMP BOSS, the cache is synced by SCU. */
	/* We can't invalidate the cache otherwise the data will be missing. */
	ambcache_clean_range((void *) srcname, strlen(srcname) + 1);
	ambcache_clean_range((void *) dstname, strlen(dstname) + 1);
#endif
 ...
 */
static void rpmsg_vffs_recv(struct rpmsg_channel *rpdev, void *data, int len,
		void *priv, u32 src)
{
...
static int rpmsg_vffs_probe(struct rpmsg_channel *rpdev)
{
	int ret = 0;
...
static void rpmsg_vffs_remove(struct rpmsg_channel *rpdev)
{
}
