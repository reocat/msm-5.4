...
static int ambarella_uport_switcher_proc_show(struct seq_file *m, void *v)
{
	int len = 0;
	...
	len += seq_printf(m, "\tport is %s: controller is %s\n\n",
		port_status, controller_status);
	...
	if (!strcasecmp(str, "host")) {
		if (ambarella_board_generic.uport_control.gpio_id >= 0) {
			...
			ambarella_set_gpio_output(
				&ambarella_board_generic.uport_control, 1);
		}
		ambarella_switch_to_host();
		...
		if (ambarella_board_generic.uport_control.gpio_id >= 0) {
			...
			ambarella_set_gpio_output(
				&ambarella_board_generic.uport_control, 0);
		}
	} else {
...
static int ambarella_uport_switcher_proc_open(
	struct inode *inode, struct file *file)
{
	return single_open(file, ambarella_uport_switcher_proc_show, NULL);
}

static const struct file_operations proc_uport_switcher_fops = {
	.open = ambarella_uport_switcher_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = ambarella_uport_switcher_proc_write,
};
