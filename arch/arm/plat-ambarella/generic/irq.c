...
void __init ambarella_init_irq(void)
{
	...
	for (i = 0; i <  NR_VIC_IRQ_SIZE; i++) {
		irq_set_chip_and_handler(VIC_INT_VEC(i),
			&ambarella_irq_chip, handle_level_irq);
		set_irq_flags(VIC_INT_VEC(i), IRQF_VALID);
	}
#if (VIC_INSTANCES >= 2)
	for (i = 0; i <  NR_VIC_IRQ_SIZE; i++) {
		irq_set_chip_and_handler(VIC2_INT_VEC(i),
			&ambarella_irq_chip, handle_level_irq);
...
#if (VIC_INSTANCES >= 3)
	for (i = 0; i <  NR_VIC_IRQ_SIZE; i++) {
		irq_set_chip_and_handler(VIC3_INT_VEC(i),
			&ambarella_irq_chip, handle_level_irq);
	...
	for (i = 0; i < AMBGPIO_SIZE; i++) {
		irq_set_chip_and_handler(GPIO_INT_VEC(i),
			&ambarella_gpio_irq_chip, handle_level_irq);
		__clear_bit(i, ambarella_gpio_wakeup_bit);
		set_irq_flags(GPIO_INT_VEC(i), IRQF_VALID);
	}
	irq_set_irq_type(GPIO0_IRQ, IRQ_TYPE_LEVEL_HIGH);
	irq_set_chained_handler(GPIO0_IRQ, ambarella_gpio_irq_handler);
	irq_set_irq_type(GPIO1_IRQ, IRQ_TYPE_LEVEL_HIGH);
	irq_set_chained_handler(GPIO1_IRQ, ambarella_gpio_irq_handler);
#if (GPIO_INSTANCES >= 3)
#if (GPIO2_IRQ != GPIO1_IRQ)
	irq_set_irq_type(GPIO2_IRQ, IRQ_TYPE_LEVEL_HIGH);
	irq_set_chained_handler(GPIO2_IRQ, ambarella_gpio_irq_handler);
#endif
#endif
#if (GPIO_INSTANCES >= 4)
	irq_set_irq_type(GPIO3_IRQ, IRQ_TYPE_LEVEL_HIGH);
	irq_set_chained_handler(GPIO3_IRQ, ambarella_gpio_irq_handler);
#endif
#if (GPIO_INSTANCES >= 5)
	irq_set_irq_type(GPIO4_IRQ, IRQ_TYPE_LEVEL_HIGH);
	irq_set_chained_handler(GPIO4_IRQ, ambarella_gpio_irq_handler);
#endif
#if (GPIO_INSTANCES >= 6)
	irq_set_irq_type(GPIO5_IRQ, IRQ_TYPE_LEVEL_HIGH);
	irq_set_chained_handler(GPIO5_IRQ, ambarella_gpio_irq_handler);
#endif
}
