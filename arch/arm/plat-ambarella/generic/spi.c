...
static struct clk *ambarella_ssi2_ahb_register_clk(void)
{
	struct clk *pgclk_ssi_ahb = NULL;
	struct clk *pgclk_ahb = NULL;

	pgclk_ssi_ahb = clk_get(NULL, "gclk_ssi2_ahb");
	if (IS_ERR(pgclk_ssi_ahb)) {
		pgclk_ahb = clk_get(NULL, "pll_out_core");
		if (IS_ERR(pgclk_ahb)) {
			BUG();
		}
		gclk_ssi2_ahb.parent = pgclk_ahb;
		ambarella_clk_add(&gclk_ssi2_ahb);
		pgclk_ssi_ahb = &gclk_ssi2_ahb;
		pr_info("SYSCLK:PWM[%lu]\n", clk_get_rate(pgclk_ssi_ahb));
	}
...
static u32 ambarella_ssi2_ahb_get_pll(void)
{
