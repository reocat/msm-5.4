...
{
	unsigned int irq = (int)(long)file_inode(file)->i_private;
...
{
	return single_open(file, irq_affinity_proc_show, inode->i_private);
}
...
{
	return single_open(file, default_affinity_show, inode->i_private);
}
