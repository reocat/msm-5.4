...
static void ambarella_ir_timer_timeout(struct timer_list *t)
{
	struct ambarella_ir_priv *priv = from_timer(priv, t, timer);
...
static int ambarella_ir_probe(struct platform_device *pdev)
{
	...
	timer_setup(&priv->timer, ambarella_ir_timer_timeout, 0);
