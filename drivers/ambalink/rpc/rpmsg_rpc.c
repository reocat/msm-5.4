...
static void wrtie_profile(void)
{
	...
	value = (unsigned int *) ambalink_phys_to_virt(addr + ambalink_shm_layout.rpc_profile_addr);
	switch (cond) {
	...
	case 4: //calculate injection time
		...
		sec_value = (unsigned int *) ambalink_phys_to_virt(result + ambalink_shm_layout.rpc_profile_addr);
