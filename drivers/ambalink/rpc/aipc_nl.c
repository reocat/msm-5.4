...
void aipc_nl_send(struct aipc_pkt *pkt, int len, int port)
{
	struct sk_buff *skb;
	struct nlmsghdr *nlh;
	int err, idx;
	...
	if(pkt->xprt.server_addr == AMBA_IPC_HOST_LINUX || pkt->xprt.server_addr == AMBA_IPC_HOST_THREADX)
		sock = nl_sock[0];
	else if(pkt->xprt.server_addr == AMBA_IPC_HOST_LINUX1 || pkt->xprt.server_addr == AMBA_IPC_HOST_LINUX2)
		sock = nl_sock[1];
	else {
		printk(KERN_ERR "%s host address error!", __func__);
		return;
	}

	
	err = nlmsg_unicast(sock, skb, port); 
	...
	if(err && (pkt->msg.type == AIPC_MSG_CALL)) {
		...
		if(pkt->xprt.server_addr == AMBA_IPC_HOST_LINUX)
			pkt->xprt.server_port = AIPC_BINDING_PORT;
		else if(pkt->xprt.server_addr == AMBA_IPC_HOST_LINUX1 || pkt->xprt.server_addr == AMBA_IPC_HOST_LINUX2)
			pkt->xprt.server_port = AIPC_BINDING_PORT2;
		...
		err = nlmsg_unicast(sock, skb, pkt->xprt.server_port); /* send it to ipcbind */ 
...
int __init aipc_nl_init(void)
{
	...
	nl_sock[0] = netlink_kernel_create(&init_net, NL_PROTO_AMBAIPC, &cfg);
	nl_sock[1] = netlink_kernel_create(&init_net, NL_PROTO_AMBAIPC2, &cfg);
...
{
	netlink_kernel_release(nl_sock[0]);
	netlink_kernel_release(nl_sock[1]);
}
