...
#if defined(CONFIG_ARCH_AMBARELLA_CV25)
#define SDIO_GLOBAL_ID 1
#else
/* use SDXC slot for WiFi */
	...
	unsigned long val, void *data)
{
	...
	case AMBA_EVENT_PRE_CPUFREQ:
		if (!sd_suspended) {
			pr_debug("%s[0x%08lx]: Pre Change\n", __func__, (unsigned long)(uintptr_t)host->regbase);
	...
	case AMBA_EVENT_POST_CPUFREQ:
		if (!sd_suspended) {
			...
			pr_debug("%s[0x%08lx]: Post Change\n", __func__, (unsigned long)(uintptr_t)host->regbase);
...
static void ambarella_sd_set_clk(struct mmc_host *mmc, u32 clock)
{
	...
	if (clock != 0) {
		...
		if (0 == mmc->index)
		{
			u32 orig_clk_reg = readw_relaxed(host->regbase + SD_CLK_OFFSET);
