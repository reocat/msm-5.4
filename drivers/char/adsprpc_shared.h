...
#define FASTRPC_IOCTL_SETPID	_IOWR('R', 22, uint32_t)
...
/* Driver should operate in virtual mode with the co-processor */
#define FASTRPC_MODE_VIRTUAL     3
...
struct fastrpc_ioctl_invoke_virtual {
	struct fastrpc_ioctl_invoke inv;
	int *fds;		/* fd list */
	unsigned int *attrs;	/* attribute list */
	unsigned int *crc;
	...
	uint64_t *perf_dsp;
	struct fastrpc_async_job *job; /* async job*/
	unsigned int gvm_fds_num;
	int *gvm_fds;		/* gvm dma handle fd list*/
};
