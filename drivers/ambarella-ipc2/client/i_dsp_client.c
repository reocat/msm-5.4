...
#include "ipcgen/i_dsp.h"
#define __IPC_CLIENT_IMPL__
#include "ipcgen/i_dsp_tab.i"
#endif
...
static CLIENT *IPC_i_dsp = NULL;
static CLIENT *IPC_i_osd = NULL;

static struct ipc_prog_s i_dsp_prog =
{
	.name = "i_dsp",
	.prog = I_DSP_PROG,
	.vers = I_DSP_VERS,
	.table = (struct ipcgen_table *) i_dsp_prog_1_table,
	.nproc = i_dsp_prog_1_nproc,
	.next = NULL,
	.prev = NULL,
};

static struct ipc_prog_s i_osd_prog =
{
	.name = "i_osd",
	.prog = I_OSD_PROG,
	.vers = I_OSD_VERS,
	.table = (struct ipcgen_table *) i_osd_prog_1_table,
	.nproc = i_osd_prog_1_nproc,
	.next = NULL,
	.prev = NULL,
};
...
int vdspdrv_refresh_osd(int id)
{
	enum clnt_stat stat;
	int rval = 0;
	struct i_osd_info osdinfo;

	DEBUG_MSG_VDSP ("[ipc] vdspdrv_refresh_osd (%d)\n", id);

	stat = i_osd_get_info_1(&id, &osdinfo, IPC_i_osd);
	if (stat == IPC_SUCCESS) {
		...
		DEBUG_MSG_VDSP ("[ipc] i_osd_get_info_1 (%d, %08x) => \n",
			   id, osd);
...
int vdspdrv_refresh(void)
{
	enum clnt_stat stat;
	int rval = 0;
	struct i_dsp_drvinfo drvinfo;
	struct i_dsp_opinfo opinfo;
	struct i_dsp_histinfo histinfo;
	int voutid;
	struct i_osd_info osdinfo;

	DEBUG_MSG_VDSP ("[ipc] vdspdrv_refresh\n");

	stat = i_dsp_active_1(NULL, &vdspdrv->active, IPC_i_dsp);
	if (stat != IPC_SUCCESS) {
		rval = -EIO;
		goto done;
	}
	DEBUG_MSG_VDSP ("[ipc] i_dsp_active_1 => %d\n", vdspdrv->active);

	stat = i_dsp_get_drvinfo_1(NULL, &drvinfo, IPC_i_dsp);
	if (stat != IPC_SUCCESS) {
		rval = -EIO;
		goto done;
	}
	...
	DEBUG_MSG_VDSP ("[ipc] i_dsp_get_drvinfo_1 => %08x %d\n",
		vdspdrv->initdata, vdspdrv->cmdsize);

	stat = i_dsp_get_opinfo_1(NULL, &opinfo, IPC_i_dsp);
	if (stat != IPC_SUCCESS) {
		rval = -EIO;
		goto done;
	}
	...
	DEBUG_MSG_VDSP ("[ipc] i_dsp_get_opinfo_1 => %d %08x %d\n",
		vdspdrv->opmode, vdspdrv->bufaddr, vdspdrv->bufsize);

	stat = i_dsp_get_histinfo_1(NULL, &histinfo, IPC_i_dsp);
	if (stat != IPC_SUCCESS) {
		rval = -EIO;
		goto done;
	}
	...
	DEBUG_MSG_VDSP ("[ipc] i_dsp_get_histinfo_1 => %08x %d %d %08x %d %d\n",
	...
	DEBUG_MSG_VDSP ("[ipc] vdspdrv_refresh END: %d\n", rval);
...
int vdspdrv_put_cmd(void *dspcmd)
{
	...
	stat = i_dsp_put_cmd_1(dspcmd, &rval, IPC_i_dsp);
	if (stat != IPC_SUCCESS)
		rval = -EIO;
...
int vdspdrv_add_capcmd(unsigned int cmd)
{
	...
	stat = i_dsp_add_capcmd_1(&cmd, &rval,IPC_i_dsp);
	if (stat != IPC_SUCCESS)
		rval = -EIO;
...
int vdspdrv_del_capcmd(unsigned int cmd)
{
	...
	stat = i_dsp_del_capcmd_1(&cmd, &rval, IPC_i_dsp);
	if (stat != IPC_SUCCESS)
		rval = -EIO;
...
{
	enum clnt_stat stat;
	struct i_dsp_recall_cmd_arg arg;
	struct i_dsp_recall_cmd_res res;
	...
	arg.voutid = voutid;
	res.dspcmd = NULL;
	stat = i_dsp_recall_cmd_1(&arg, &res, IPC_i_dsp);
...
int vdspdrv_takeover(void)
{
	...
	stat = i_dsp_takeover_1(NULL, &rval, IPC_i_dsp);
	if (stat != IPC_SUCCESS)
		return -EIO;
...
int vdspdrv_handback(void)
{
	...
	stat = i_dsp_handback_1(NULL, &rval, IPC_i_dsp);
	if (stat != IPC_SUCCESS)
		return -EIO;
...
int vdspdrv_video_plane_enabled(int voutid)
{
	...
	stat = i_osd_video_plane_enabled_1(&voutid, &rval, IPC_i_osd);
	if (stat != IPC_SUCCESS)
		return -EIO;
...
int vdspdrv_en_video_plane(int voutid, int enable)
{
	...
	arg.voutid = voutid;
	arg.enable = enable;
	stat = i_osd_en_video_plane_1(&arg, &rval, IPC_i_osd);
	if (stat != IPC_SUCCESS)
		return -EIO;
...
int vdspdrv_osd_suspend(void)
{
	...
	stat = i_osd_suspend_activities_1(NULL, &rval, IPC_i_osd);
	if (stat != IPC_SUCCESS)
		return -EIO;
...
int vdspdrv_osd_resume(void)
{
	...
	stat = i_osd_resume_activities_1(NULL, &rval, IPC_i_osd);
	if (stat != IPC_SUCCESS)
		return -EIO;
...
{
	enum clnt_stat stat;
	struct i_osd_clut res;

	DEBUG_MSG_VDSP ("[ipc] vdspdrv_osd_get_clut (%d)\n", voutid);

	stat = i_osd_get_clut_1(&voutid, &res, IPC_i_osd);
	if (stat != IPC_SUCCESS)
		return NULL;
...
int vdspdrv_osd_set_clut(int voutid, void *clut)
{
	...
	arg.voutid = voutid;
	arg.clut = clut;
	stat = i_osd_set_clut_1(&arg, &rval, IPC_i_osd);
	if (stat != IPC_SUCCESS)
		return -EIO;
...
int vdspdrv_osd_enable(int voutid, int enable)
{
	...
	arg.voutid = voutid;
	arg.enable = enable;
	stat = i_osd_enable_1(&arg, &rval, IPC_i_osd);
	if (stat != IPC_SUCCESS)
		return -EIO;
...
int vdspdrv_osd_flip(int voutid, int flip)
{
	...
	arg.voutid = voutid;
	arg.flip = flip;
	stat = i_osd_flip_1(&arg, &rval, IPC_i_osd);
	if (stat != IPC_SUCCESS)
		return -EIO;
...
int vdspdrv_osd_setbuf(int voutid, void *ptr)
{
	enum clnt_stat stat;
	struct i_osd_setbuf_arg arg;
	int rval;

	DEBUG_MSG_VDSP ("[ipc] vdspdrv_osd_setbuf (%d, %08x)\n", voutid, ptr);

	arg.voutid = voutid;
	arg.ptr = ptr;
	stat = i_osd_setbuf_1(&arg, &rval, IPC_i_osd);
	if (stat != IPC_SUCCESS)
		return -EIO;

	return rval;
}
EXPORT_SYMBOL(vdspdrv_osd_setbuf);

int vdspdrv_osd_apply(int voutid, struct vdspdrv_osd *osd)
{
	enum clnt_stat stat;
	struct i_osd_apply_arg arg;
	int rval;

	DEBUG_MSG_VDSP ("[ipc] vdspdrv_osd_apply (%d, %08x)\n", voutid, osd);
	DEBUG_MSG_VDSP ("enabled = %d\n", osd->enabled);
	DEBUG_MSG_VDSP ("interlaced = %d\n", osd->interlaced);
	DEBUG_MSG_VDSP ("colorformat = %d\n", osd->colorformat);
	DEBUG_MSG_VDSP ("bitsperpixel = %d\n", osd->bitsperpixel);
	...
	osd->zbuf0 = (u32) ipc_virt_to_phys ((u32) osd->zbuf0);
	osd->zbuf1 = (u32) ipc_virt_to_phys ((u32) osd->zbuf1);
	osd->zbuf2 = (u32) ipc_virt_to_phys ((u32) osd->zbuf2);
	osd->zbuf3 = (u32) ipc_virt_to_phys ((u32) osd->zbuf3);
	osd->osdupdptr = (u32) ipc_virt_to_phys ((u32) osd->osdupdptr);

	ambcache_clean_range (osd, sizeof (*osd));

	arg.voutid = voutid;
	arg.info = (struct i_osd_info *) osd;
	stat = i_osd_apply_1(&arg, &rval, IPC_i_osd);
	if (stat != IPC_SUCCESS)
		return -EIO;

	return rval;
}
EXPORT_SYMBOL(vdspdrv_osd_apply);
 ...
 */
static void i_dsp_procfs_init(void)
{
 ...
 */
static void i_dsp_procfs_cleanup(void)
{
	if (proc_vdspdrv_osd1_plane) {
 ...
 */
static int __init i_dsp_init(void)
{
	int rval = 0;
	...
	IPC_i_dsp = ipc_clnt_prog_register(&i_dsp_prog);
	if (IPC_i_dsp == NULL) {
		rval = -EPERM;
		goto done;
	}

	IPC_i_osd = ipc_clnt_prog_register(&i_osd_prog);
	if (IPC_i_osd == NULL) {
		rval = -EPERM;
		goto done;
	}
	...
	if (rval < 0) {
		if (IPC_i_dsp) {
			ipc_clnt_prog_unregister(&i_dsp_prog, IPC_i_dsp);
			IPC_i_dsp = NULL;
		}
		if (IPC_i_osd) {
			ipc_clnt_prog_unregister(&i_osd_prog, IPC_i_osd);
			IPC_i_osd = NULL;
		}
	}
 ...
 * Client exit.
 */
static void __exit i_dsp_cleanup(void)
{
	if (IPC_i_dsp) {
		ipc_clnt_prog_unregister(&i_dsp_prog, IPC_i_dsp);
		IPC_i_dsp = NULL;
...
MODULE_DESCRIPTION("Ambarella IPC: i_dsp.x");
