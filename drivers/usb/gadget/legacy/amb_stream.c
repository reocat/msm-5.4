 ...
 *
 * Copyright (C) 2009-2029, Ambarella, Inc.
 *
...
static struct usb_endpoint_descriptor ss_intr_in_desc = {
	.bLength =		USB_DT_ENDPOINT_SIZE,
	.bDescriptorType =	USB_DT_ENDPOINT,
	.bEndpointAddress =	USB_DIR_IN,
	.bmAttributes =		USB_ENDPOINT_XFER_INT,
...
static struct usb_ss_ep_comp_descriptor ss_intr_in_comp_desc = {
	.bLength =		sizeof(ss_intr_in_comp_desc),
	.bDescriptorType =	USB_DT_SS_ENDPOINT_COMP,

	/* the following 3 values can be tweaked if necessary */
	/* .bMaxBurst =		0, */
	/* .bmAttributes =	0, */
	.wBytesPerInterval = cpu_to_le16(AG_NOTIFY_MAXPACKET),
};

static struct usb_descriptor_header *ss_amb_data_stream_function[] = {
	(struct usb_descriptor_header *) &amb_data_stream_intf,
	...
	(struct usb_descriptor_header *) &ss_intr_in_desc,
	(struct usb_descriptor_header *) &ss_intr_in_comp_desc,
	NULL,
};
...
static ssize_t ag_read(struct file *file, char __user *buf,
...
static ssize_t ag_write(struct file *file, const char __user *buf,
	size_t count, loff_t *ppos)
{
	int rval, size, len = 0;
	struct amb_dev *dev = ag_device;
