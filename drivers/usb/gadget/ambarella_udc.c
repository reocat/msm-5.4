		...
		gfp_t gfp_flags)
{
	...
	if (list_empty(&ep->queue)) {
		...
		case USB_DIR_OUT:
			...
			break;
		default:
			pr_err("%s: bulk or intr queue error\n", __func__);
			BUG();
			return -EL2HLT;
		}
	}
...
static int ambarella_udc_set_halt(struct usb_ep *_ep, int value)
{
	...
	if (ep->ep.desc /* not ep0 */
		&& (ep->ep.desc->bmAttributes & 0x03) == USB_ENDPOINT_XFER_ISOC){
		pr_err("%s: %s, -EINVAL 2\n", __func__, _ep->name);
		return -EINVAL;
	}
...
{
	unsigned long flags;
	struct ambarella_udc *udc = container_of(gadget, struct ambarella_udc, gadget);
