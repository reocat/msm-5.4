...
static irqreturn_t mxt_read_messages_t44(struct mxt_data *data)
{
	...
	if (count == 0) {
		dev_err(dev, "Interrupt triggered but zero messages\n");
		return IRQ_NONE;
	...
	if (ret < 0) {
		dev_err(dev, "Unexpected invalid message\n");
		return IRQ_NONE;
	}
...
int mxt_download_config(struct mxt_data *data, const char *fn)
{
	...
	while (pos < cfg->size) {
		 ...
		 * We warn here but do nothing else - the malloc has zeroed
	...
	pos = 0;
	while (pos < config_mem_size) {
		size = config_mem_size - pos;

		if (size > MXT_MAX_BLOCK_WRITE)
			size = MXT_MAX_BLOCK_WRITE;

		ret = mxt_write_block(data->client, config_start_offset + pos,
				      size, config_mem + pos);
		if (ret != 0) {
