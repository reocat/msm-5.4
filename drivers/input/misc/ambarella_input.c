...
static int ambarella_gpio_read_sw(struct ambarella_key_table *pkey_sw)
{
	int gpio_id;
	int sw_level;
	int sw_val = 0;

	gpio_id = pkey_sw->gpio_sw.id;
	sw_level = ambarella_gpio_get(gpio_id);
	if (pkey_sw->gpio_sw.active_val == sw_level) {
		sw_val = 1;
	}
...
static int ambarella_vi_proc_show(struct seq_file *m, void *v)
{
	int len = 0;
	struct ambarella_input_board_info *pbinfo;
	int i;

	pbinfo = (struct ambarella_input_board_info *)m->private;
	if (!pbinfo->pkeymap) {
		goto ambarella_vi_proc_show_exit;
	}
	...
	for (i = 0; i < AMBINPUT_TABLE_SIZE; i++) {
		...
		if (pbinfo->pkeymap[i].type == AMBINPUT_GPIO_SW) {
			len += seq_printf(m, "SW[%d]=%d\n",
				pbinfo->pkeymap[i].gpio_sw.key_code,
				ambarella_gpio_read_sw(&pbinfo->pkeymap[i]));
		}
	}

ambarella_vi_proc_show_exit:
	return len;
}
	...
	const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	pbinfo = (struct ambarella_input_board_info *)PDE_DATA(
		file_inode(file));
...
static int ambarella_vi_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, ambarella_vi_proc_show, PDE_DATA(inode));
}

static const struct file_operations ambarella_vi_fops = {
	.open = ambarella_vi_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = ambarella_vi_proc_write,
};
#endif
	...
	struct ambarella_input_board_info *pbinfo)
{
	...
	if (ambarella_keymap != NULL) {
		ret_val = request_firmware(&ext_key_map,
	...
	for (i = 0; i < AMBINPUT_TABLE_SIZE; i++) {
		...
		case AMBINPUT_SOURCE_GPIO:
			...
			if ((ret_val == 0) &&
				...
				ret_val = irq_set_irq_wake(GPIO_INT_VEC(
...
static void ambarella_free_keymap(struct ambarella_input_board_info *pbinfo)
{
	int i;
...
static int ambarella_input_probe(struct platform_device *pdev)
{
	...
	ret_val = input_register_device(pinput_dev);
	if (ret_val) {
	...
	ret_val = ambarella_setup_keymap(pbinfo);
	if (ret_val) {
		goto ambarella_input_probe_unregister_device;
	}

#ifdef CONFIG_PROC_FS
	pbinfo->input_proc = proc_create_data(AMBVI_NAME,
		(S_IRUGO | S_IWUSR), get_ambarella_proc_dir(),
		&ambarella_vi_fops, pbinfo);
#endif
...
static int __init ambarella_input_init(void)
{
	...
	ret_val = platform_driver_register(&ambarella_input_driver);
	if (ret_val) {
		pr_err("Register ambarella_input_driver = %d!\n", ret_val);
	}
	if (pboard_info == NULL) {
		ret_val = -1;
