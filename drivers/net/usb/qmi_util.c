...
)
{
  ...
  txn = (qmi_txn_hdr_type *) kmalloc (txn_size,GFP_KERNEL);
...
)
{
  ...
  */
  if ((delete) || (txn->ready_to_delete == TRUE))
  {
...
)
{
 ...
 // QMI_PLATFORM_MUTEX_LOCK (list_mutex);
  ...
  qmi_util_release_txn_no_lock(txn, delete, list_head);
