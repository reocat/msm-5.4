...
{
	u32 bit_pos = IEEE80211_EHT_PPE_THRES_INFO_HEADER_SIZE, val = 0;
	u8 nss, ru, i;
	...
	for (nss = 0; nss <= ppet->numss_m1; nss++) {
		for (ru = 0; ru < hweight8(IEEE80211_EHT_PPE_THRES_RU_INDEX_BITMASK_MASK);
		     ru++) {
			if ((ppet->ru_bit_mask & BIT(ru)) == 0)
				continue;

			for (i = 0; i < IEEE80211_EHT_PPE_THRES_INFO_PPET_SIZE; i++) {
				val >>= 1;
