...
static int ath11k_dp_rx_pdev_srng_alloc(struct ath11k *ar)
{
	...
	if (ret) {
		ath11k_warn(ab, "failed to setup rx_refill_buf_ring\n");
		return ret;
	}
	...
	for (i = 0; i < NUM_RXDMA_PER_PDEV; i++) {
		ret = ath11k_dp_srng_setup(ab, &dp->rxdma_err_dst_ring[i],
					   HAL_RXDMA_DST, 0, dp->mac_id + i,
					   DP_RXDMA_ERR_DST_RING_SIZE);
		if (ret) {
			ath11k_warn(ab, "failed to setup rxdma_err_dst_ring %d\n", i);
			return ret;
		}
	}
	...
	ret = ath11k_dp_srng_setup(ab, &dp->rxdma_mon_dst_ring,
	...
	ret = ath11k_dp_srng_setup(ab, &dp->rxdma_mon_desc_ring,
