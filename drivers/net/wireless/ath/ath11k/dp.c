			   ...
			   int budget)
{
	...
	while (ab->ring_mask.tx_ring_mask[grp_id] >> i) {
		if (ab->ring_mask.tx_ring_mask[grp_id] & BIT(i))
			ath11k_dp_tx_completion_handler(ab, i);
		i++;
	}

	if (ab->ring_mask.rx_err_ring_mask[grp_id]) {
	...
	if (ab->ring_mask.rx_wbm_rel_ring_mask[grp_id]) {
	...
	if (ab->ring_mask.rx_ring_mask[grp_id]) {
		i =  fls(ab->ring_mask.rx_ring_mask[grp_id]) - 1;
	...
	if (ab->ring_mask.reo_status_ring_mask[grp_id])
