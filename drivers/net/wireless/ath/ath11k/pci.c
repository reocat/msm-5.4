...
static int ath11k_pci_claim(struct ath11k_pci *ab_pci, struct pci_dev *pdev)
{
	...
	if (ret) {
		ath11k_err(ab, "failed to set pci dma mask (%d): %d\n",
			   ret, ATH11K_PCI_DMA_MASK);
		goto release_region;
	}

	ret = pci_set_consistent_dma_mask(pdev, DMA_BIT_MASK(ATH11K_PCI_DMA_MASK));
	if (ret) {
		ath11k_err(ab, "failed to set pci consistent dma mask (%d): %d\n",
			   ret, ATH11K_PCI_DMA_MASK);
		goto release_region;
	}
	...
	ab_pci->mem_len = pci_resource_len(pdev, ATH11K_PCI_BAR_NUM);
	ab_pci->mem = pci_iomap(pdev, ATH11K_PCI_BAR_NUM, 0);
	if (!ab_pci->mem) {
			    ...
			    const struct pci_device_id *pci_dev)
{
	...
	case QCA6390_DEVICE_ID:
		val = ath11k_pci_read32(ab, TCSR_SOC_HW_VERSION);
		ath11k_dbg(ab, ATH11K_DBG_PCI, "tcsr_soc_hw_version 0x%x\n", val);

		if (FIELD_GET(TCSR_SOC_HW_VERSION_MAJOR_MASK, val) == 1)
