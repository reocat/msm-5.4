 ...
 *
 * $Id: dhd_sdio.c 726203 2020-06-19 02:58:05Z $
 */
...
typedef struct dhd_bt_dev {
	void *bt_data;
	bs_int_handler bt_sdio_int_callback;
...
#if defined(BT_OVER_SDIO)
static dhd_pub_t	*g_dhd_pub = NULL;
static void bt_sdio_int_handler(dhd_bus_t *bus);
#endif /* (BT_OVER_SDIO) */
	...
	if (!DHD_NOPMU(bus)) {
		...
		if (!si_setcore(bus->sih, ARMCR4_CORE_ID, 0)) {
			...
			case BCM4373_CHIP_ID:
				...
				{
					uint fn = 2;
#if defined(CUSTOM_SDIO_F2_BLKSIZE)
				/* Since we do not pull the chip support to Makefile for 4373 chipset
				 * We have to consider if the CUSTOM_SDIO_F2_BLKSIZE is defined by user
				 */
					fn = fn << SDIO_FUNC_BLOCK_SIZE_SHIFT | CUSTOM_SDIO_F2_BLKSIZE;
#else
					fn = fn << SDIO_FUNC_BLOCK_SIZE_SHIFT | F2_BLOCK_SIZE_256;
					sd_f2_blocksize = F2_BLOCK_SIZE_256;
#endif
...
#ifdef BT_OVER_SDIO
void bt_sdio_int_handler(dhd_bus_t *bus)
{
	bt_dev_t *btdev = bus->bt_dev;

	if (btdev != NULL && btdev->bt_sdio_int_callback != NULL)
		btdev->bt_sdio_int_callback(btdev->bt_data);
}

wlan_bt_handle_t bt_sdio_attach(void *btdata, bs_int_handler bs_int_fun)
{
	bt_dev_t *btdev;
	dhd_bus_t *bus;
	wlan_bt_handle_t handle = NULL;

	if (g_dhd_pub == NULL || (g_dhd_pub->up == 0)) {
		goto done;
	}
	bus = g_dhd_pub->bus;
	/* Allocate bt dev */
	if (!(btdev = MALLOC(g_dhd_pub->osh, sizeof(bt_dev_t)))) {
		DHD_ERROR(("%s: MALLOC of bt_dev_t failed\n", __FUNCTION__));
		goto done;
	}

	bzero(btdev, sizeof(bt_dev_t));

	btdev->bt_data = btdata;
	btdev->bt_sdio_int_callback = bs_int_fun;
	bus->bt_dev = btdev;
	handle = (wlan_bt_handle_t) g_dhd_pub;
done:
	return handle;

} EXPORT_SYMBOL(bt_sdio_attach);

void bt_sdio_detach(wlan_bt_handle_t handle)
{
	dhd_pub_t *dhd_pub = (dhd_pub_t *)handle;
	dhd_bus_t *bus = dhd_pub->bus;
	bt_dev_t *btdev = bus->bt_dev;

	if (dhd_pub && btdev) {
		MFREE(dhd_pub->osh, btdev, sizeof(bt_dev_t));
		bus->bt_dev = NULL;
	}
} EXPORT_SYMBOL(bt_sdio_detach);

int dhd_get_wlan_info(wlan_bt_handle_t handle, btsdio_info_t *bs_info)
{
	int ret = 0;
	dhd_pub_t *dhdp = (dhd_pub_t *)handle;

	if (handle != NULL && bs_info != NULL) {
#ifdef SSDIO_BUF_BT
		bs_info->bt_buf_reg_addr = SI_ENUM_BASE + OFFSETOF(chipcregs_t,gci_input[6]);
#else
		bs_info->wlan_buf_reg_addr = SI_ENUM_BASE + OFFSETOF(chipcregs_t,gci_output[2]);
#endif /* SSDIO_BUF_BT */
		bs_info->host_ctrl_reg_addr = SI_ENUM_BASE + OFFSETOF(chipcregs_t, gci_output[3]);
		bs_info->bt_ctrl_reg_addr = SI_ENUM_BASE + OFFSETOF(chipcregs_t, gci_input[7]);

#ifdef SSDIO_BUF_BT
		DHD_ERROR(("%s, bt_buf_reg_addr : 0x%x\n", __FUNCTION__, bs_info->bt_buf_reg_addr));
#else
		DHD_ERROR(("%s, wlan_buf_reg_addr : 0x%x\n", __FUNCTION__, bs_info->wlan_buf_reg_addr));
#endif	/* SSDIO_BUF_BT */

		DHD_ERROR(("%s, host_ctrl_reg_addr: 0x%x\n", __FUNCTION__, bs_info->host_ctrl_reg_addr));
		DHD_ERROR(("%s, bt_ctrl_reg_addr: 0x%x\n", __FUNCTION__, bs_info->bt_ctrl_reg_addr));

#ifndef SSDIO_BUF_BT
		ret = dhd_iovar(dhdp, 0, "bus:btsdiobufaddr", NULL, 0, (char *)&bs_info->wlan_buf_addr,
				sizeof(bs_info->wlan_buf_addr), FALSE);
		if (ret < 0) {
			DHD_ERROR(("%s: Fail to get btsdio buffer address\n",
				__FUNCTION__));
		}
#endif /* !SSDIO_BUF_BT */
	} else
	...
	return ret;
} EXPORT_SYMBOL(dhd_get_wlan_info);

uint32 dhd_bus_reg_read(void *h, uint32 addr)
{
	uint32 rval;
	...
	return rval;
} EXPORT_SYMBOL(dhd_bus_reg_read);

void dhd_bus_reg_write(void *h, uint32 addr, uint32 val)
{
	dhd_pub_t *dhdp = (dhd_pub_t *)h;
        dhd_bus_t *bus = (dhd_bus_t *)dhdp->bus;
	...
	dhd_os_sdunlock(bus->dhd);
} EXPORT_SYMBOL(dhd_bus_reg_write);

int
dhd_membytes(void *h, bool set, uint32 address, uint8 *data, uint size)
{
	int ret;
	dhd_pub_t *dhdp = (dhd_pub_t *)h;
        dhd_bus_t *bus = (dhd_bus_t *)dhdp->bus;
        ...
        dhd_os_sdlock(bus->dhd);
	ret = dhd_bus_membytes(dhdp, set, address, data, size);
        dhd_os_sdunlock(bus->dhd);
	return ret;
} EXPORT_SYMBOL(dhd_membytes);
