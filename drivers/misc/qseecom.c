...
free_buf:
	...
	if (ret) {
		...
		if (partition_name) {
			qcom_ice_umap_key_partition_name(slot,
							partition_name);
		}

		ret = qcom_ice_free_key_slot(slot);
		if (ret)
...
free_buf:
	...
	if (ret) {
		if (wipe_info.partition_name) {
			ret = qcom_ice_map_key_partition_name_pwd(slot,
