...
int ambxc_eof;
int output_callback(char **addr, int *size, int *count, void **priv)
{
	struct ambxc_ctx *ctx = g_dev->ctx;
	struct vb2_buffer *vb;

	if (!ctx) {
	    printk(KERN_DEBUG "ambxc: OUTPUT device is not enabled\n");
	    return 0;
	}
	...
	if (ambxc_eof) {
		...
		*size = 0;
		*count = 0;
		dprintk(ctx->dev, "OUTPUT stream has been marked EOS, queue NULL buffer to itron\n");
		return 1;
	}
...
void output_callback_done(void *priv)
{
...
static int capture_fillbuff(struct ambxc_ctx *ctx)
{
	struct vb2_buffer *vb;
	u8 *p;
	int n = 0;
...
static int capture_thread(void *data)
{
	...
	for (;;) {
		...
		capture_fillbuff(ctx);
	}

	return 0;

	// v4l2_m2m_job_finish(st_dev->m2m_dev, curr_ctx->m2m_ctx);
}
...
static int ambxc_open(struct file *file)
{
	...
	atomic_inc(&dev->ref_cnt);
	dev->ctx = ctx;
	dev->kthread = kthread_run(capture_thread, dev, dev->v4l2_dev.name);
