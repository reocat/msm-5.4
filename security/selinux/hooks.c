...
static void cred_init_security(void)
{
	...
	tsec->state = get_selinux_ns(init_selinux_ns);
}
	...
	const struct task_struct *p)
{
	...
	while (tsec->state != current_selinux_ns && tsec->parent_cred)
		tsec = selinux_cred(tsec->parent_cred);
	if (tsec->state != current_selinux_ns)
...
{
	int rc = security_context_str_to_sid(current_selinux_ns, s,
					     sid, GFP_KERNEL);
	if (rc)
...
static int show_sid(struct seq_file *m, u32 sid)
{
	...
	rc = security_sid_to_context(current_selinux_ns, sid,
					     &context, &len);
	if (!rc) {
...
#define INITCONTEXTLEN 255
	...
	rc = security_context_to_sid_default(current_selinux_ns, context, rc, sid,
					     def_sid, GFP_NOFS);
	if (rc) {
				       ...
				       void **value, size_t *len)
{
	...
	if (!current_selinux_ns->initialized || !(sbsec->flags & SBLABEL_MNT))
		return -EOPNOTSUPP;
					...
					struct kernfs_node *kn)
{
	...
	rc = security_context_to_sid(current_selinux_ns, context, clen, &parent_sid,
				     GFP_KERNEL);
	kfree(context);
	if (rc)
		return rc;
	...
	if (tsec->create_sid) {
		...
		rc = security_transition_sid(current_selinux_ns, tsec->sid,
			     ...
			     unsigned long prot, unsigned long flags)
{
	...
	if (current_selinux_ns->checkreqprot)
		prot = reqprot;
				...
				gfp_t gfp)
{
	...
	*tsec = *old_tsec;
	tsec->state = get_selinux_ns(old_tsec->state);
...
static int selinux_nlmsg_perm(struct sock *sk, struct sk_buff *skb)
{
	...
	while (data_len >= nlmsg_total_size(0)) {
		...
		if (rc == 0) {
			...
			if (enforcing_enabled(current_selinux_ns) &&
...
static unsigned int selinux_ipv4_forward(void *priv,
					 struct sk_buff *skb,
					 const struct nf_hook_state *ns)
{
	return selinux_ip_forward(skb, ns->in, PF_INET);
}
...
static unsigned int selinux_ipv6_forward(void *priv,
					 ...
					 const struct nf_hook_state *ns)
{
	return selinux_ip_forward(skb, ns->in, PF_INET6);
}
#endif	/* IPV6 */
...
static unsigned int selinux_ipv4_postroute(void *priv,
					   ...
					   const struct nf_hook_state *ns)
{
	return selinux_ip_postroute(skb, ns->out, PF_INET);
}
...
static unsigned int selinux_ipv6_postroute(void *priv,
					   ...
					   const struct nf_hook_state *ns)
{
	return selinux_ip_postroute(skb, ns->out, PF_INET6);
}
#endif	/* IPV6 */
...
static int selinux_setprocattr(const char *name, void *value, size_t size)
{
	...
	if (!strcmp(name, "exec")) {
		...
		if (sid) {
			error = avc_has_perm(current_selinux_ns, mysid, sid,
...
static int selinux_perf_event_open(struct perf_event_attr *attr, int type)
{
	...
	return avc_has_perm(current_selinux_ns, sid, sid, SECCLASS_PERF_EVENT,
			    requested, NULL);
}
...
static int selinux_perf_event_read(struct perf_event *event)
{
	...
	return avc_has_perm(current_selinux_ns, sid, perfsec->sid,
