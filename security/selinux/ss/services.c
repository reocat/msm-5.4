			 ...
			 struct extended_perms *xperms)
{
	...
	context_struct_compute_av(policydb, scontext, tcontext, tclass, avd,
				  xperms);
	map_decision(&ns->ss->map, orig_tclass, avd,
...
int security_sidtab_hash_stats(struct selinux_ns *ns, char *page)
{
	int rc;
	...
	read_lock(&ns->ss->policy_rwlock);
	rc = sidtab_hash_stats(ns->ss->sidtab, page);
	read_unlock(&ns->ss->policy_rwlock);
 ...
 */
int security_sid_to_context_inval(struct selinux_ns *ns, u32 sid,
				  char **scontext, u32 *scontext_len)
{
	return security_sid_to_context_core(ns, sid, scontext,
					    scontext_len, 1, 1);
...
static int context_struct_to_sid(struct selinux_ns *ns,
					...
					int force)
{
	...
	*sid = SECSID_NULL;
	...
	if (rc == -EINVAL && force) {
		...
		goto out_unlock;
	rc = context_struct_to_sid(ns, &context, sid);
				...
				bool kern)
{
	...
	rc = context_struct_to_sid(ns, &newcontext, out_sid);
out_unlock:
	read_unlock(&ns->ss->policy_rwlock);
...
static int convert_context(struct context *oldc, struct context *newc, void *p)
{
	...
	if (!policydb_context_isvalid(args->newp, newc)) {
		rc = convert_context_handle_invalid_context(args->ns, oldc);
		if (rc)
			goto bad;
	}
...
static void security_load_policycaps(struct selinux_ns *ns)
{
	...
	ns->android_netlink_route = p->android_netlink_route;
	selinux_nlmsg_init();
}
		      ...
		      u8 protocol, u16 port, u32 *out_sid)
{
	...
	if (c) {
		if (!c->sid[0]) {
			rc = context_struct_to_sid(ns, &c->context[0],
						   &c->sid[0]);
			if (rc)
				goto out;
		}
		       ...
		       char *name, u32 *if_sid)
{
	...
	if (c) {
		if (!c->sid[0] || !c->sid[1]) {
			...
			if (rc)
				goto out;
			rc = context_struct_to_sid(ns, &c->context[1],
						   &c->sid[1]);
			if (rc)
				goto out;
		}
	...
	*nel = 0;
	...
	ebitmap_for_each_positive_bit(&user->roles, rnode, i) {
		...
		ebitmap_for_each_positive_bit(&role->types, tnode, j) {
			...
			rc = context_struct_to_sid(ns, &usercon, &sid);
			if (rc)
				goto out_unlock;
			if (mynel < maxnel) {
				mysids[mynel++] = sid;
				       ...
				       u32 *sid)
{
	...
	*sid = SECINITSID_UNLABELED;
	...
	if (!c->sid[0]) {
		rc = context_struct_to_sid(ns, &c->context[0], &c->sid[0]);
		if (rc)
			goto out;
	}
			  ...
			  u32 sid, u32 mls_sid, u32 *new_sid)
{
	...
	rc = context_struct_to_sid(ns, &newcon, new_sid);
out_unlock:
	read_unlock(&ns->ss->policy_rwlock);
				   ...
				   u32 *sid)
{
	...
	if (secattr->flags & NETLBL_SECATTR_CACHE)
		...
		*sid = secattr->attr.secid;
		...
		rc = context_struct_to_sid(ns, &ctx_new, sid);
		if (rc)
			goto out_free;
