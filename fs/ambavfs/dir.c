 ...
 */
#ifdef CONFIG_ARM
#define AMBAFS_READDIR_MAGIC	0xfeadbeef
#else
#define AMBAFS_READDIR_MAGIC	LLONG_MAX
#endif
static int ambafs_dir_readdir(struct file *file, struct dir_context *ctx)
{
	...
	} else if (file->private_data != (void *) AMBAFS_READDIR_MAGIC) {
		file->f_pos = (loff_t)(uintptr_t)file->private_data;
	} else {
		return 0;
	}
	...
	if (msg->flag)
		...
		if (fill_dir_from_msg(dir_db, dir, ctx)) {
			file->private_data = (void *)(uintptr_t)file->f_pos;
			return 0;
		}
	}
...
ls_exit:
	...
	file->private_data = (void *) AMBAFS_READDIR_MAGIC;
	return 0;
}
