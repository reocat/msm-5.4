...
static void inline prepare_read_msg(struct ambafs_msg *msg, struct file *file)
{
	...
	io->fp = (u64)(uintptr_t) file->private_data;
	io->total = 0;
}
				...
				struct page **pages, int nr_pages, void *fp)
{
	...
	for (page_idx = 0; page_idx < nr_pages; page_idx++) {
		...
		AMBAFS_DMSG("%s: i_size = %d, page_offset = %lld, io->bh[%d].len = %d phy=%lld \r\n",
			  __func__, (int) page->mapping->host->i_size,	page_offset(page), page_idx, io->bh[page_idx].len, (u64)io->bh[page_idx].addr);
		AMBAFS_DMSG("%s: %s \r\n", __func__, (char *) ambalink_phys_to_virt((u64) io->bh[page_idx].addr));

	}

	msg->cmd = AMBAFS_CMD_WRITE;
	io->fp = (u64)(uintptr_t) fp;
	io->total = nr_pages;
